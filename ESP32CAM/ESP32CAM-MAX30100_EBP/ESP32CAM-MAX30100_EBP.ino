 ...
 * 
 * Este programa lee el sensor I2C Max30100 y envía los valores de 
 * ritmo cardiaco (HR) y oxigenación (SpO2) por MQTT. Toma la bilbioteca
 * MAX30105 del Sparkfun.
 * 
 * Componente     PinESP32CAM     Estados lógicos
 * ledStatus------GPIO 33---------On=>LOW, Off=>HIGH
 * ledFlash-------GPIO 4----------On=>HIGH, Off=>LOW
 * 
 * SensorMAX30100 ESP32CAM        Resistores
 * Vin            5V
 * GND            GND
 * SDA            14
 * SCL            15
...
const char* password = "mz7srSbma6";  // Aquí debes poner la contraseña de tu red

//Datos del broker MQTT
const char* mqtt_server = "192.168.1.231"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
IPAddress server(192,168,1,231);

// Objetos
WiFiClient espClient; // Este objeto maneja los datos de conexion WiFi
PubSubClient client(espClient); // Este objeto maneja los datos de conexion al broker
MAX30105 particleSensor; //Objeto para el sensor

//Inicio de variables y constantes del sensor MAX30100
...
//Fin de variables y constantes del sensor MAX30100
...
//Variables de manejo del sensor
long timeLast2;//Tracker de tiempo de lecturas del sensor
int waitMax30100 = 1000;//Tiempo entre lecturas del sensor

// Inicialización del programa
void setup() {
  // Iniciar comunicación serial
  Serial.begin (115200);
  //Configuracion de los leds indicadores de conexion
  pinMode (flashLedPin, OUTPUT);
  pinMode (statusLedPin, OUTPUT);
  digitalWrite (flashLedPin, LOW);
  digitalWrite (statusLedPin, HIGH);
  //Configuracion de los leds indicadores del sensor
  ...
  //Inicio de secuencias no bloqueantes
...
void loop() {
  ...
  //Inicio de lectura del sensor
  if (timeNow - timeLast2 > waitMax30100) {
    timeLast2 = timeNow;

    //Aqui va el codigo de lectura del sensor presente en el ejemplo
    ...
    //Fin de codigo del lectura del sensor presente en el ejemplo
  }
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("codigoIoT/SIC/mqtt", char_array); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
